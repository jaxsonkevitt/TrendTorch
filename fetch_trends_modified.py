...
def detect_spike_modified(trend_data):
    ...
    try:
        ...
        print(f"Error in detect_spike_modified: {str(e)}")
        return 0
...
def get_trending_searches():
  ...
  "Outdoor Adventure Gear": ["GPS Watches", "Multi-Tools", "Portable Water Filters", "Thermal Jackets"]
    ...
    # Try to add today's top searches
    try:
        today_searches = pytrends.today_searches(pn='US')
        trending_searches.update([sanitize_query(search) for search in today_searches])
        print(f"Added {len(today_searches)} today's top searches")
    except Exception as e:
        print(f"Couldn't get today's top searches: {str(e)}")
        ...
        while retries < 3:  # Reduced max retries to 3
            try:
                ...
                if interest_over_time is not None and not interest_over_time.empty:
                    ...
                    spike_percentage = detect_spike_modified(trend_data)  # Use the modified function
...
if __name__ == "__main__":
    ...
    with open('trending_searches_modified.json', 'w', encoding='utf-8') as f:
        json.dump(trends, f, ensure_ascii=False, indent=4)
    
    # Save to CSV for easy manual review
    save_to_csv(trends, 'trending_searches_modified.csv')
    
    print(f"Analysis complete. Found {len(trends)} trending searches.")
    print("Results saved to 'trending_searches_modified.json' and 'trending_searches_modified.csv'.")
